...
try {
    ...
    // Corrigir qualquer utilizador admin existente (independente do email)
    $pdo->prepare("UPDATE users SET email='admin@example.com', password_hash=?, role_id=?, role='admin', active=1 WHERE email IN ('admin@pap.local','admin@example.com') LIMIT 1")->execute([$hash, $role_id]);
    ...
    if (!$exists) {
        ...
        echo '<div class="step ok">✓ <span>Utilizador <code>admin@example.com</code> pronto (senha: <code>admin123</code>)</span></div>';
    }
} catch (Exception $e) {
    echo '<div class="step warn">⚠ <span>Erro admin: ' . htmlspecialchars($e->getMessage()) . '</span></div>';
}
?>
